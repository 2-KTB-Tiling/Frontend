        ...
        stage('Deploy to EC2 with Docker Compose') {
            steps {
                script {
                    ...
                    # 🔹 버전 업데이트
                    grep -q "^NEW_TAG_FRONTEND=" /home/ubuntu/.env && \
                        sudo sed -i "s/^NEW_TAG_FRONTEND=.*/NEW_TAG_FRONTEND=${newTag}/" /home/ubuntu/.env || \
                        echo "NEW_TAG_FRONTEND=${newTag}" | sudo tee -a /home/ubuntu/.env
